...
class Gemini(LLM):
    ...
    def create_prompt(self, metadata_lmm, metadata_detectron, query, premodel_context):
        prompt = f'''You are a helpful AI assistant who takes input from the user which is the query. You answer the query with the help of the following additional information -
            ...
            if the user is asking for information about a specific object, verify if it is in METADATA_DETECTRON first, if not ignore the premodel_result tell the object is absent in the image and you need further information.
            The correctness of the premodel_context depends on if the objects stated to be present in the image in the premodel_context are actually present in the METADATA_DETECTRON if not premodel_context is wrong and you can tell you cannot answer the question since the context is absent.
            ...
            It should be as if youre prediction is soley from a single model.
